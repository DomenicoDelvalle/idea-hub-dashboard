...
import type { AppIdea, ResearchData, BlueprintData, ProjectManagementData } from '../types';
import { getResearchData, getBlueprintDataForIdea, getProjectManagementDataForSdd } from '../services/supabaseService';
...
export const Dashboard: React.FC<DashboardProps> = ({ selectedAppIdea }) => {
  ...
  const fetchDataForIdea = useCallback(async (ideaId: string) => {
    try {
      ...
      if (blueprint?.sddDocuments && blueprint.sddDocuments.length > 0) {
        const projectDataPromises = blueprint.sddDocuments.map(sdd => getProjectManagementDataForSdd(sdd.id));
        const projectResults = await Promise.all(projectDataPromises);

        const combinedProjectData: ProjectManagementData = { projects: [], tasks: [] };
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      <header>
        <h1 className="text-3xl font-bold text-white">{selectedAppIdea.idea_text}</h1>
        <p className="text-slate-400 mt-1">
          Created on: {new Date(selectedAppIdea.created_at).toLocaleDateString()}
        </p>
      </header>
