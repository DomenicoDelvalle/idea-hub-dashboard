...
const ConceptDisplay: React.FC<{ concept: Record<string, any> }> = ({
  concept,
}) => (
  <div className="space-y-4 md:space-y-6">
    {concept.suggested_name && (
      <div>
        <p className="text-xs md:text-sm font-semibold text-slate-400 mb-1">
          Suggested Name
        </p>
        <p className="text-xl md:text-2xl font-bold text-sky-300 break-words">
          {concept.suggested_name}
        </p>
      </div>
    )}
    ...
    {concept.unique_value_proposition && (
      <div>
        <p className="text-xs md:text-sm font-semibold text-slate-400 mb-2">
          Unique Value Proposition
        </p>
        <blockquote className="border-l-4 border-sky-500 pl-3 md:pl-4 italic text-slate-300 bg-slate-900 p-2 md:p-3 rounded-r-lg text-sm md:text-base">
          {concept.unique_value_proposition}
        </blockquote>
      </div>
    )}
          ...
          <ul className="space-y-1 md:space-y-2">
            {concept.core_features.map((feature: string, index: number) => (
              <li key={index} className="flex items-start">
                <svg
                  className="w-4 h-4 md:w-5 md:h-5 text-green-400 mr-2 mt-0.5 flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-4 md:gap-x-6 gap-y-3 md:gap-y-4 pt-2">
...
const CompetitorCard: React.FC<{ competitor: Competitor }> = ({
  competitor: c,
}) => (
  <div
    key={c.id}
    className="p-4 bg-slate-900 rounded-lg border border-slate-800 transition-all hover:border-slate-700/80"
  >
      ...
      <div>
        ...
        {c.app_store_url && (
          <a
            ...
            className="text-xs text-slate-400 hover:text-sky-400 hover:underline"
          >
            App Store Link
          </a>
        )}
          ...
          <ul className="space-y-1.5 text-slate-400">
            {c.strengths.map((strength, i) => (
              <li key={i} className="flex items-start">
                <span className="text-green-500/80 mr-2 mt-1 flex-shrink-0 text-xs">
                  &#x25CF;
                </span>
        ...
        {Array.isArray(c.weaknesses) && c.weaknesses.length > 0 ? (
          <ul className="space-y-1.5 text-slate-400">
            {c.weaknesses.map((weakness, i) => (
              <li key={i} className="flex items-start">
                <span className="text-red-500/80 mr-2 mt-1 flex-shrink-0 text-xs">
                  &#x25CF;
                </span>
    ...
    {c.user_review_summary && (
      <div className="mt-4 pt-3 border-t border-slate-800">
        <h5 className="font-semibold text-slate-300 mb-1 text-sm">
          User Review Summary
        </h5>
        <blockquote className="text-sm text-slate-400 italic border-l-2 border-slate-700 pl-3">
          "{c.user_review_summary}"
        </blockquote>
      </div>
    )}
...
const PsychographicTags: React.FC<{
  title: string;
  items: any;
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2">
        {itemList.map((item, index) => (
          <span
            key={index}
            className={`flex items-center px-2.5 py-1 text-xs font-semibold rounded-full ${color}`}
          >
            {icon}
...
const PersonaCard: React.FC<{ persona: TargetPersona }> = ({ persona: p }) => {
  ...
  const demoItems = [
    {
      icon: <CalendarIcon className="text-slate-400" />,
      value: demographics.age || demographics.primary_age_range,
    },
    {
      icon: <LocationMarkerIcon className="text-slate-400" />,
      value: demographics.location || demographics.geographic_location,
    },
    {
    ...
    {
      icon: <AcademicCapIcon className="text-slate-400" />,
    ...
    <div className="bg-slate-900 rounded-lg border border-slate-800 p-5 transition-all hover:border-slate-700/80">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
        ...
        <div className="space-y-4">
          <PsychographicTags
            title="Goals"
            items={psychographics.goals}
            icon={<TargetIcon />}
            color="bg-green-500/10 text-green-300"
          />
          <PsychographicTags
            title="Values"
            items={psychographics.values}
            icon={<HeartIcon />}
            color="bg-purple-500/10 text-purple-300"
          />
          <PsychographicTags
            title="Interests"
            items={psychographics.interests}
            icon={<SparklesIcon />}
            color="bg-yellow-500/10 text-yellow-300"
          />
          <PsychographicTags
            title="Challenges"
            items={psychographics.challenges}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {data.unifiedProductBriefing && (
            <Card title="Unified Product Briefing" className="lg:col-span-2">
              <div className="space-y-6">
                <MarkdownRenderer
                  markdown={
                    data.unifiedProductBriefing.executive_summary_markdown
                  }
                />
                ...
                <div>
                  <h4 className="font-bold mb-4 text-slate-100 text-lg">
                    Final App Concept
                  </h4>
                  <ConceptDisplay
                    concept={data.unifiedProductBriefing.final_app_concept}
                  />
                </div>
              </div>
            </Card>
          ...
          {data.features.length > 0 && (
            <Card title="Proposed Features">
              <ul className="space-y-2">
                {data.features.map((f) => (
                  <li key={f.id}>
                    <strong className="text-slate-100">{f.feature_name}</strong>{" "}
                    ({f.category}):{" "}
                    <span className="text-slate-400">{f.justification}</span>
                  </li>
                ))}
              </ul>
            </Card>
            ...
            <Card title="ASO Assets" icon={<TagIcon />}>
              <div className="space-y-6">
                <div>
                  ...
                  {Array.isArray(data.asoAssets.name_suggestions) &&
                  data.asoAssets.name_suggestions.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {data.asoAssets.name_suggestions.map((name, i) => (
                        <span
                          key={i}
                          className="px-3 py-1 text-sm font-semibold rounded-full bg-sky-500/10 text-sky-300 border border-sky-500/20"
                        >
                          {String(name)}
                        </span>
                      ))}
                ...
                <div>
                  ...
                  {Array.isArray(data.asoAssets.subtitle_suggestions) &&
                  data.asoAssets.subtitle_suggestions.length > 0 ? (
                    <ul className="space-y-2">
                      {data.asoAssets.subtitle_suggestions.map(
                        (subtitle, i) => (
                          <li
                            key={i}
                    ...
                    <p className="text-slate-500 italic">
                      No subtitle suggestions.
                    </p>
                  )}
                </div>

                <div>
                  <h4 className="font-bold mb-3 text-slate-100">
                    App Description Preview
                  </h4>
                  <div className="p-4 border border-slate-700/80 rounded-lg bg-slate-900">
                    <MarkdownRenderer
                      markdown={data.asoAssets.app_description}
                    />
                  </div>
                </div>
              </div>
          ...
          {data.monetizationModel && (
            <Card title="Monetization Model" className="lg:col-span-2">
              <ColumnarMarkdownReport
                markdown={data.monetizationModel.report_markdown}
              />
            </Card>
          )}
          ...
          {data.visualBriefing && (
            <Card title="Visual Briefing" className="lg:col-span-2">
              <TabbedMarkdownReport
                markdown={data.visualBriefing.briefing_markdown}
              />
            </Card>
          )}
        </div>
