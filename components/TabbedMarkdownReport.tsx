...
const getCleanMarkdown = (markdown: string): string => {
  ...
  try {
    ...
    if (typeof parsed === "object" && parsed !== null) {
      ...
      if (
        "briefing_markdown" in parsed &&
        typeof parsed.briefing_markdown === "string"
      ) {
        return parsed.briefing_markdown;
      }
      if (
        "design_brief_markdown" in parsed &&
        typeof parsed.design_brief_markdown === "string"
      ) {
...
export const TabbedMarkdownReport: React.FC<TabbedMarkdownReportProps> = ({
  markdown,
}) => {
    ...
    for (const line of lines) {
      if (line.startsWith("## ")) {
        ...
        if (currentSection) {
          ...
          if (introTitleMatch) {
            parsedSections.push({
              title: introTitleMatch[1].trim(),
              content: introContent.replace(/^# .*\n/, "").trim(),
            });
          } else {
            parsedSections.push({
              title: "Overview",
  ...
  if (sections.length <= 1) {
    // If there's 0 or 1 section, no need for tabs
  ...
  return (
    <div>
      <div className="border-b border-slate-700 mb-4">
        <nav
          className="-mb-px flex space-x-2 md:space-x-4 overflow-x-auto scrollbar-hide"
          aria-label="Tabs"
        >
          {sections.map((section, index) => (
            <button
              ...
              } whitespace-nowrap py-2 md:py-3 px-2 md:px-4 border-b-2 font-medium text-xs md:text-sm transition-colors duration-200 flex-shrink-0`}
            >
              {section.title}
            </button>
          ))}
