...
export const Sidebar: React.FC<SidebarProps> = ({ appIdeas, selectedAppIdeaId, onSelectIdea, loading, error }) => {
  return (
    <nav className="flex-1 overflow-y-auto p-2">
      <h2 className="text-xs font-bold uppercase text-slate-500 px-2 mb-2">App Ideas</h2>
        ...
        <ul>
          {appIdeas.map(idea => (
            <li key={idea.id}>
              ...
              >
                <span className="flex-1 pr-2">{idea.idea_text}</span>
                <span className={`w-3 h-3 rounded-full ${statusColorMap[idea.status]}`} title={idea.status}></span>
              </button>
            </li>
          ))}
        </ul>
